...
const reRequestLine = new RegExp("^MRCP/(?<version>([0-9]+)\.([0-9]+))[ \t]+([0-9]+)[ \t]+(?<method>[-A-Z]+)[ \t]+(?<request_id>[0-9]+)$")

// Response Message start-line: MRCP/2.0 message-length request-id status-code request-state
const reResponseLine = new RegExp("^MRCP/(?<version>([0-9]+)\.([0-9]+))[ \t]+([0-9]+)[ \t]+(?<request_id>[0-9]+)[ \t]+(?<status_code>[0-9]+)[ \t]+(?<request_state>[-A-Z]+)$")

// Event Message start-line: MRCP/2.0 message-length event-name request-id request-state
const reEventLine = new RegExp("^MRCP/(?<version>([0-9]+)\.([0-9]+))[ \t]+([0-9]+)[ \t]+(?<event_name>[-A-Z]+)[ \t]+(?<request_id>[0-9]+)[ \t]+(?<request_state>[-A-Z]+)$")
